...
Items 1, 2 and 5 landed. The remaining ones are ordered to minimize total work rather than by number (the amendment lifted the strict order): 3, then 4 and 6 together, since those two share one slider-page implementation. Each is closed out (firmware pushed, CI green, docs, master bump) before the next starts. Delete each item here as it lands.
