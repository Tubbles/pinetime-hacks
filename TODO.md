...
Flashing fails again (closed 2026-08-10 after two clean flashes; reopened by report, no cause known yet). Root-causing runs on the on-watch BLE trace, captured per the runbook section 1: BEFORE rebooting the watch (a reboot wipes the RAM ring), write 0x02 to characteristic `00080003` (nRF Connect, GB T disconnected first), page the reads out, decode with `tools/decode_trace.py`. Waiting on: the captured hex, the exact phone-side failure observable (error message / stuck point), the GB T file log of the failed attempt, and whether the watch rebooted since the failure. 2026-09-17: another failure; the first read-out attempt hit the read-out protocol's own bug (a 200-byte page per callback breaks under Android's Read Blob continuation at the default MTU, see LOG.md 2026-09-17), so the runbook now says to request MTU 517 first. That write was 0x02, so `/trace.bin` on the watch holds the failed flash's snapshot regardless of what the RAM ring loses. Firmware follow-up: make the 00080003 read-out MTU-safe, e.g. page at most `ble_att_mtu(connHandle) - 2` bytes per callback (the callback already receives the connection handle) so no MTU can trigger a blob continuation, and consider a write value that serves `/trace.bin` instead of the RAM ring.
