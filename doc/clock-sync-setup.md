...
On the PineTime device in Gadgetbridge T, open Device settings -> Developer settings / BLE Intent API and:
...
- Set the BLE-characteristics filter to `00080001-78fc-48fe-8e23-433b3a1942d0` (key tones). If clock sync is un-parked, add `00070002-78fc-48fe-8e23-433b3a1942d0` (ClockSync state), comma-separated. The filter governs notify subscriptions only; do NOT list write-only characteristics like the ClockSync control `00070001` (writes need no filter, and a subscription attempt on them is a pointless extra GATT action).
- Set the BLE-API package filter, comma-separated: `com.tubbles.phone.debug` (the current deployment; key tones only) — add `com.tubbles.deskclock.debug` if clock sync is un-parked.
