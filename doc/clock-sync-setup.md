...
The Intent API is required for the key-tones feature even with clock sync parked: the door digits and the watch's 'E' hang-up ride the notify direction, and the InCall auto-open for OUTGOING calls is Phone T's Intent-API write to `00080002` (Gadgetbridge cannot see outgoing calls on modern Android; incoming-call auto-open is native and needs none of this). The Intent API targets exactly ONE package, which decides the topology: with clock sync parked, target Phone T directly and Clock T is not needed at all (its KeyToneReceiver listens for Gadgetbridge's own broadcast action and validates the characteristic itself); with clock sync active, target Clock T, which then forwards key tones to Phone T (the hub pattern).
...
- Set the BLE-API package filter: `com.tubbles.phone.debug` (clock sync parked, the current deployment) or the clock fork's applicationId `com.tubbles.deskclock.debug` (clock sync active, hub mode).
