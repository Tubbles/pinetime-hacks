...
- Reconnect the watch afterward (the notify subscription happens at service discovery). If the ClockSync characteristics don't show up, or a DFU fails with "GATT REQ NOT SUPPORTED" (error 6): Android's GATT service cache is stale, and the ONLY reliable way to clear it is to unpair the watch in Android's Bluetooth settings ("Forget device") and then re-add it in Gadgetbridge. Removing the device inside Gadgetbridge, toggling Bluetooth, or even uninstalling/reinstalling Gadgetbridge does NOT clear it — the cache lives in the OS Bluetooth stack, keyed to the bond (field-verified twice on 2026-08-02). The fingerprint of a stale cache in a log is service "discovery" completing in tens of milliseconds instead of seconds.
