...
The Intent API is required for the key-tones feature even with clock sync parked: the door digits and the watch's 'E' hang-up ride the notify direction (`00080001` -> Clock T hub -> Phone T), and the InCall auto-open for OUTGOING calls is Phone T's Intent-API write to `00080002` (Gadgetbridge cannot see outgoing calls on modern Android; incoming-call auto-open is native and needs none of this). Clock T also stays installed regardless of clock sync — it is the forwarding hub, since the Intent API targets exactly one package.
