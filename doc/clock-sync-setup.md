...
Flashing a watch that still runs firmware WITHOUT the host-task stack fix (pre-`0f3a9b22`; see LOG.md 2026-08-03): that firmware can soft-brick its own OTA channel at runtime (RAM-only CCCD corruption, healed by reboot — the watch itself is never at risk). Procedure that avoids it: (1) reboot the watch (hold the button until reset); (2) let Gadgetbridge T connect, then disconnect and reconnect ONCE — the corruption risk lives in the boot's FIRST disconnect (the deep bond-persist path runs once per boot/pairing and is skipped afterward), so this burns it while nothing depends on the connection; (3) flash. If it still fails, reboot and repeat. Firmware from `0f3a9b22` onward removes the corruption at the source, and re-pairing is never the fix — it makes this specific problem more likely, not less.
