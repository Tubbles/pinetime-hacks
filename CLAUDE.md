...
- **Never build Android apps locally on this machine** (standing user rule, 2026-08-02): local Gradle runs OOM-killed user services and froze the user's SSH session twice, and heap caps did not help. All APKs come from CI — `phone-app.yml` (artifact `clocksync-clock-apk`) and `phone-dialer.yml` (artifact `keytones-dialer-apk`; builds `assembleFossDebug`, the foss flavor of three). Iterate on CI failures by pushing fixes. Single-file `javac` checks are fine. Prefer CI for the firmware too; a solo local podman compile check has been tolerated when quick iteration is needed.
