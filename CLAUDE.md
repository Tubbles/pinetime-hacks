...
- APKs are built by CI: `phone-app.yml` (artifact `clocksync-clock-apk`), `phone-dialer.yml` (artifact `keytones-dialer-apk`; builds `assembleFossDebug`, the foss flavor of three), and `gadgetbridge.yml` (artifact `gadgetbridge-t-apk`; builds `:app:assembleMainlineDebug` with the repo's own Gradle 9.5.1 wrapper inside the `.#gadgetbridge` shell, JDK 21). The Android builds are memory-hungry (Gradle heaps up to 4 GiB), so on modest machines push and let CI build rather than building locally.
