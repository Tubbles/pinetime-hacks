...
- APKs are built by CI: `phone-app.yml` (artifact `clocksync-clock-apk`) and `phone-dialer.yml` (artifact `keytones-dialer-apk`; builds `assembleFossDebug`, the foss flavor of three). The dialer build is memory-hungry (Fossify sets a 4 GiB Gradle heap and the dex step needs it), so on modest machines push and let CI build rather than building locally.
