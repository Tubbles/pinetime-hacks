...
Hardware context that shapes everything: the watch is **sealed** (no SWD access), so flashing is OTA/DFU-only via Gadgetbridge. Firmware safety (bootloader intact, validation/rollback flow understood) is a hard requirement before any flash. Record findings about this in the project log (`doc/log/`).

## Documentation and process

Project docs live in `doc/` (design documents, research notes, the log); `TODO.md` and `SUGGESTIONS.md` stay in the repo root. The project log is one file per date under `doc/log/` (append today's findings to `doc/log/<today>.md`, creating it on first entry), with `doc/LOG.md` as the index (one line per entry). It records learnings and decisions (BLE gotchas, flashing findings, dead ends) that the commits and code do not capture; skim the index and open only the relevant day files before debugging hardware or protocols.

**Progressive disclosure.** A central part of how documentation is written here: structure it so a reader — human or LLM with a finite context window — can load only what the task at hand needs. Concretely: spread information into distinct, focused `.md` files instead of growing monoliths; keep index files (like `doc/LOG.md`) that link with one-line hooks rather than inlining content; split the log by date; and add `CLAUDE.md` files to subdirectories around the repo carrying the context specific to that area, so working in a subtree pulls in that subtree's knowledge and nothing else. When a document grows past the point where a reader must scan unrelated material to find what they came for, split it and leave an index behind.
...
**Finished work is assumed to work** (user decision 2026-08-10): when an item is code-complete, CI-green, and documented, close it out fully — do not leave per-feature verification checklists or "pending field verification" states behind in `TODO.md` or `README.md`. The user field-tests in daily use and reports anything broken as they discover it; a report reopens the item then. Exception: in an active back-and-forth (e.g. hammering out a bug across flash-test iterations), the user's on-device confirmation stays part of the loop — the item is not done until they say the fix holds.

Close out every work item by documenting it before you commit. Put the learnings, gotchas, dead ends, verified facts, and the reasoning behind decisions in the project log (`doc/log/<today>.md` + an index line in `doc/LOG.md`): the git diff records what changed, the log records what you found out and why. Delete finished items from `TODO.md` once their record is in the log, and keep the unfinished ones. The documentation pass is part of the work, not an optional extra.

**Single source of truth.** Every fact lives in exactly one file; every other document links to it instead of restating it. Ownership: the feature list and per-feature status belong to `README.md`; outstanding/parked work and the reasoning behind status changes to `TODO.md`; history and findings to the project log, `doc/log/` (an append-only journal — recording an event there is never duplication); setup and configuration steps to `doc/clock-sync-setup.md`. When a status changes, update the owning file and leave the others alone. Writing one decision into three files is the antipattern this rule exists to prevent: every copy is a future inconsistency.

## Repo layout

- `CLAUDE.md` — this file.
- `README.md` — short project overview.
- `doc/` — design documents, research reports; `doc/log/` per-date log files with `doc/LOG.md` as the index.
